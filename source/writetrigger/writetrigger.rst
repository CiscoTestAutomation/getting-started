...
.. csv-table:: Files required to run triggers
    ...
    "Job file", "Python (.py)", "Imports ``gRun`` functionality from the ``harness`` package. Points to a specific datafile and identifies which triggers to run."
...
#. Triggers inherit from the base :monospace:`Trigger` class, which contains common setup and cleanup tasks and verifications. For more information about common setup and cleanup, see the topic :ref:`auto-testing-process` in the *Get Started with pyATS* guide.
#. Setup steps typically check for any prerequisites and do the configuration required for the test case.
#. If the setup steps fail (under :monospace:`@aetest.setup`), the rest of the trigger does not run.
#. The |library| triggers typically have a test step ("shut") and a recovery test step ("unshut").

Have a look at the following example, and then we'll explain it step-by-step.
...
#. Define the setup steps to:
   ...
   .. note:: If any of the setup steps fail, the trigger stops.
...
.. code-block:: python
  ...
  import os
   ...
   def main():
       ...
       gRun(trigger_uids=Or('TriggerShutNoShutBgp'),
           trigger_datafile='new_trigger_datafile.yaml')


Example of a trigger with verifications
---------------------------------------
The simple trigger example verifies the expected results. 
